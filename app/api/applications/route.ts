...
function generateTrackingCode() {
  ...
  return `EHI-${new Date().getFullYear()}-${suffix}`;
}
...
async function parseRequestRequest(request: Request) {
...
export async function POST(request: Request) {
  try {
    ...
    const { fields, formData } = await parseRequestRequest(request);
    ...
    try {
      ...
      logProductionError("Request email notification failed or timed out", emailError);
    }
    ...
    logProductionError("Request submission failed", error);
    return jsonError("Something went wrong while saving your request. Please try again.", 500);
  }
}
