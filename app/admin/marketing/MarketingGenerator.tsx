...
const ctaDetails = {
  ...
  address: "Shop 30, Napex Car Park, By Computer Village, Victoria Island, Lagos, Nigeria",
};

const topicAngles: Record<Topic, string[]> = {
  "Laptop Sales": [
    "China laptop buying preparation",
    "business, tourist, student, and family visit IT solution guidance",
    "invitation letter and itinerary support",
  ],
  "Laptop Accessories": [
    "UK visitor Tech readiness",
    "clear financial and usage-purpose detailation",
    "family visit, business, study, and tourism support",
  ],
  "Laptop Repairs": [
    "Canada Tech and study permit planning",
    "proof of funds and request file organization",
    "usage-purpose and sponsor-detail guidance",
  ],
  "RAM/SSD Upgrades": [
    "USA appointment preparation",
    "usage-purpose clarity and supporting details",
    "interview-readiness guidance",
  ],
  "Software Installation": [
    "Dubai tech detailation",
    "tourism and short-stay planning",
    "reservation and itinerary support",
  ],
  "Corporate Supply": [
    "school request and study device preparation",
    "statement of purpose and sponsor-detail planning",
    "student detail checklist guidance",
  ],
  "Laptop Buying Tips": [
    "smarter tech preparation",
    "booking, itinerary, and detailation reminders",
    "practical tech planning support",
  ],
  "Common Laptop Buying Mistakes": [
    "common Tech refusal risks",
    "inconsistent forms and weak detailation",
    "how organized files reduce avoidable mistakes",
  ],
  "Accessory Bundles": [
    "detail compilation and organization",
    "laptop specs, budget, accessories, and setup notes",
    "clean request file preparation",
  ],
};
...
function toneIntro(tone: Tone) {
  const intros: Record<Tone, string> = {
    Professional: "Plan your next request with structured guidance.",
    Friendly: "Thinking about your next computer setup? Let us help you prepare with less stress.",
    Urgent: "Do not wait until your tech date is close before organizing your details.",
    Educational: "A strong request starts with clear, complete, and consistent details.",
  };
...
function hashtagFor(topic: Topic) {
  return `#EhisTech #TechSupport #ITSolutions #${topic.replace(/[^A-Za-z]/g, "")} #NigeriaTech`;
}

function callToAction(topic: Topic) {
  return `Contact Ehi's Tech Computer Services for ${topic} support. Call ${ctaDetails.phone}, WhatsApp ${ctaDetails.whatsapp}, or visit ${ctaDetails.website}.`;
}
...
function dailyCaption(platform: Platform, topic: Topic, tone: Tone, dayIndex = 0) {
  ...
  return `${toneIntro(tone)} Ehi's Tech Computer Services assists with ${angle}. ${platformNotes[platform]} Visit us at ${ctaDetails.address}.`;
}

function imageIdea(topic: Topic, platform: Platform, index = 0) {
  const ideas = [
    `${topic} checklist graphic with laptop, laptop setup, and tech icons`,
    `Short ${platform} visual showing key details arranged neatly on a desk`,
    `Client-ready tech detail carousel for ${topic}`,
...
export function MarketingGenerator() {
  ...
  const [topic, setTopic] = useState<Topic>("Laptop Sales");
