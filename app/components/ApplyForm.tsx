...
const fileFields = [
  { name: "passportUpload", label: "Device Photo" },
  { name: "passportPhoto", label: "Receipt or Invoice" },
  { name: "bankStatement", label: "Accessory Photo" },
  { name: "supportingDocument", label: "Additional File" },
] as const;
...
export function ApplyForm() {
    ...
    return new Promise<{ application: { trackingCode: string } }>((resolve, reject) => {
      ...
      request.onload = () => {
        ...
        reject(
          new Error(
            data.error ??
              "Unable to submit request. Please try again or contact us on WhatsApp."
          )
        );
      };
      request.onerror = () =>
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNotice("Your request has been submitted successfully. Save your tracking code.");
    } catch (error) {
      setStatus("error");
      setUploadProgress(0);
      setNotice(
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-2xl font-bold text-[#073b7a]">Request details</h2>
        <p className="mt-2 text-sm leading-6 text-slate-600">
          Complete the required fields and attach any available supporting files.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-2 text-sm font-semibold text-slate-700">
          ...
          <input
            ...
            type="text"
            placeholder="Laptop, repair, upgrade, accessory, or supply"
      ...
      <label className="grid gap-2 text-sm font-semibold text-slate-700">
        Usage or support type
          ...
          >
            ...
            <option>Student Use</option>
            <option>Business Use</option>
            <option>Gaming or Design</option>
      ...
      <label className="grid gap-2 text-sm font-semibold text-slate-700">
        ...
        <textarea
          ...
          name="message"
          placeholder="Tell us about the laptop, accessory, repair issue, upgrade, quantity, or support you need."
      ...
      <div className="rounded border border-blue-100 bg-blue-50 p-4 md:p-5">
        ...
        <p className="mt-1 text-sm leading-6 text-slate-600">
          Upload photos, receipts, device images, or supporting files. We will automatically optimize them for review.
        </p>
      ...
      {status === "submitting" ? (
        <div className="rounded bg-blue-50 p-4 ring-1 ring-blue-100">
          <div className="flex items-center justify-between text-sm font-semibold text-[#073b7a]">
            <span>Saving request and uploading files</span>
